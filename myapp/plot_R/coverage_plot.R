...
plot_coverage_by_method <- function(data, true_value, nominal = 0.95) {
  ...
  # Y limits: data-driven (don’t force start at 0); pad slightly and clip to [0,1]
  ...
  # Major breaks every 0.05 across the visible range; no minor breaks
  brk_start <- floor(lims[1] * 20) / 20
  brk_end <- ceiling(lims[2] * 20) / 20
  brks <- seq(brk_start, brk_end, by = 0.05)

  # Facet labels to match the bias plot
  ...
  ggplot2::ggplot(cov_summary, ggplot2::aes(x = method, y = coverage)) +
    ...
    ggplot2::scale_y_continuous(
      ...
      breaks = brks,
      minor_breaks = NULL,
      expand = ggplot2::expansion(mult = c(0, 0)) # keep the padding we set in 'lims'
    ) +
    ggplot2::labs(
    ...
    ggplot2::theme(
      ...
      # theme_bw() gridlines are kept so you get lines at every 0.05 tick
    )
}
